...
int main(int argc, char** argv)
{
    ...
    streamCfg.channels[TRXDir::Rx] = { 0, 1 };
    streamCfg.channels[TRXDir::Tx] = { 0, 1 };
    ...
    while (std::chrono::high_resolution_clock::now() - startTime < std::chrono::seconds(10) && !stopProgram)
    {
        ...
        if (t2 - t1 > std::chrono::seconds(1))
        {
            ...
            for (std::size_t c = 1; c < streamCfg.channels.at(TRXDir::Rx).size(); ++c)
                gp.writef(", '-' with points title 'ch %lu'\n", c);
            for (std::size_t c = 0; c < streamCfg.channels.at(TRXDir::Rx).size(); ++c)
            {
