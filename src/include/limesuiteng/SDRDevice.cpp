...
OpStatus SDRDevice::I2CWrite(uint32_t bus, uint32_t soc, uint32_t offset, uint8_t offset_len, const uint8_t* data, uint32_t length)
{
    return ReportError(OpStatus::NotImplemented, "WriteI2C not implemented"s);
}

OpStatus SDRDevice::I2CRead(uint32_t bus, uint32_t soc, uint32_t offset, uint8_t offset_len, uint8_t* data, uint32_t length)
{
