#ifndef LIME_Xillybus_H
#define LIME_Xillybus_H
...
class LIME_API Xillybus : public ISerialPort
{
  public:
    ...
    OpStatus Open(const std::filesystem::path& write_path, const std::filesystem::path& read_path, bool nonblock);
    OpStatus Reset();
    ...
    virtual int Write(const uint8_t* data, std::size_t length, int timeout_ms);
    virtual int Read(uint8_t* data, std::size_t length, int timeout_ms);
  ...
  private:
    ...
    std::filesystem::path reader_path;
    bool nonblocking;
};

} // namespace lime

#endif // LIME_Xillybus_H
