...
bool FX3::Connect(uint16_t vid, uint16_t pid, const std::string_view serial)
{
    Disconnect();
#ifndef __unix__
    std::scoped_lock lock{ FX3mutex };
...
void FX3::WaitForXfers(uint8_t endPointAddr)
{
...
int FX3::GetUSBContextIndex()
{
    ...
    {
        if (!FX3contexts[i].isTransferUsed)
        {
            contextFound = true;
            break;
        }
    ...
    FX3contexts[i].isTransferUsed = true;
