...
OEMTestData LimeSDR_XTRX::GNSSTest(OEMTestReporter& reporter)
{
    ...
    if (gw.version == 1) // original gateware
    {
        ...
        if (test.status != OpStatus::Success)
            test.output = ToString(test.status);
        return test;
    }
    else if (gw.version >= 2) // litex based gateware
    {
        ...
        constexpr int bytesToRead = 512;
        char inMessage[bytesToRead + 16]; // + some padding
        memset(inMessage, 0, sizeof(inMessage));

        // read out any data that's already in Rx fifo
        read(tty_fd, inMessage, 64);
        ...
        {
            test.output = "Expected GNSS message not found";
    ...
    {
        test.output = "Unexpected gateware version.";
        test.status = OpStatus::Error;
        return test;
    }
    return test;
...
OEMTestData LimeSDR_XTRX::RFTest(OEMTestReporter& reporter)
{
    ...
    // XTRX
    // channel A
    inputs.push_back({ 1, 1000e6, "Band2", txGain, "LNAL", rxLNALgain, -33 });
    inputs.push_back({ 1, 2000e6, "Band2", txGain, "LNAW", rxLNAWgain, -28 });
    inputs.push_back({ 1, 3500e6, "Band1", txGain, "LNAH", rxLNAHgain, -29 });
    // channel A
    inputs.push_back({ 1, 1000e6, "Band2", txGain, "LNAL", rxLNAWgain, -33 });
    inputs.push_back({ 1, 2000e6, "Band2", txGain, "LNAW", rxLNAWgain, -28 });
    inputs.push_back({ 1, 3500e6, "Band1", txGain, "LNAH", rxLNAWgain, -29 });
