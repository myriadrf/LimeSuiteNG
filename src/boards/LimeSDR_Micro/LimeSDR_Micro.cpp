    ...
    , mConfigInProgress(false)
{
    ...
    if (!la9310->IsM4CoreProgrammed())
    {
        ...
        fw = std::make_shared<LA9310_FW_Impl>(streamingPort);
        ...
        if (status == OpStatus::Success && !la9310->IsVSPAFirmwareLoaded())
        {
            std::vector<char> firmware;
    ...
    iqstreamer = std::make_shared<LA9310_IQStreamer>(fw);
    if (status == OpStatus::Success)
    {
...

    {
        ...
        uint32_t boardRefClk = fw ? fw->GetReferenceClock() : 0;
        if (boardRefClk > 0)
        {
    ...
    desc.socTree->children.push_back(std::make_shared<DeviceTreeNode>(iqstreamer.get(), "LA9310"s));
...
static OpStatus SetLA9310SamplingRate(std::shared_ptr<LA9310_FW_Impl> fw, double sampleRate, uint32_t oversample)
{
    ...
    OpStatus status = fw->SetSystemClock(systemClock, adc_divider_mask, dac_divider_mask);
    if (status != OpStatus::Success)
...
OpStatus LimeSDR_Micro::Configure(const SDRConfig& cfg, uint8_t socIndex)
{
    OpStatus status;
    if (!fw)
        return ReportError(OpStatus::Error, "Firmware is not loaded");
    ...
    if (cfg.referenceClockFreq > 0)
    {
        ...
        status = fw->SetReferenceClock(cfg.referenceClockFreq, external);
        if (status != OpStatus::Success)
...
OpStatus LimeSDR_Micro::SetDCOffset(uint8_t moduleIndex, TRXDir trx, uint8_t channel, const complex64f_t& offset)
{
    ...
    // if (trx == TRXDir::Tx)
    //     return fw->GetTxDCCorrector(channel)->SetDCOffset(offset);
    // else
    //     return fw->GetRxDCCorrector(channel)->SetDCOffset(offset);
    return OpStatus::NotImplemented;
}
...
OpStatus LimeSDR_Micro::SetIQBalance(uint8_t moduleIndex, TRXDir trx, uint8_t channel, const complex64f_t& balance)
{
    ...
    // if (trx == TRXDir::Tx)
    //     return fw->GetTxQEC(channel)->SetGainCorrection(ratio);
    // else
    //     return fw->GetRxQEC(channel)->SetGainCorrection(ratio);
    return OpStatus::NotImplemented;
}
    ...
    eMemoryDevice device, uint8_t moduleIndex, const char* data, size_t length, UploadMemoryCallback callback)
{
    ...
    case eMemoryDevice::ARM_M4: {
        ...
        status = fw->LoadFirmware(std::span<const char, std::dynamic_extent>(data, length));
        if (status != OpStatus::Success)
            return status;
...
{
    if (!la9310->IsVSPAFirmwareLoaded())
    {
        std::vector<char> firmware;
        const std::string vspafirmware_path = "/lib/firmware/"s + limesdrmicro::vspa_firmware_name_default;
        OpStatus status = limesdrmicro::ReadFileIntoVector(vspafirmware_path, firmware);
        ...
        lime::info("Programming LimeSDR-Micro VSPA " + vspafirmware_path);
        status = fw->LoadVSPAFirmware(firmware);
        if (status != OpStatus::Success)
            return nullptr;
    }

    auto stream = std::make_unique<LA9310_TRX>(iqstreamer, mStreamingPort);
    ...
    OpStatus status = stream->Setup(config_mod);
    if (status != OpStatus::Success)
    {
...
double LimeSDR_Micro::GetSampleRate(uint8_t moduleIndex, TRXDir trx, uint8_t channel, uint32_t* rf_samplerate)
{
    ...
    fw->GetADCDACRates(&adcRate, &dacRate);
    const auto vspa_ch = 2; // TODO: la9310->vspa.api_channel_remap(channel);
