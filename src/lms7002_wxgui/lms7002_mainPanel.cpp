...
void lms7002_mainPanel::UpdateVisiblePanel()
{
    ...
    if (pageId == ID_TAB_SXR) //change active channel to A
        ...
        chip->Modify_SPI_Reg_bits(LMS7_MAC, 2);
    else
        chip->Modify_SPI_Reg_bits(LMS7_MAC, rbChannelA->GetValue() == 1 ? 1 : 2);
...
void lms7002_mainPanel::OnOpenProject(wxCommandEvent& event)
{
    ...
    wxCommandEvent tevt;
    // LMS_WriteParam(sdrDevice, LMS7_MAC, rbChannelA->GetValue() == 1 ? 1 : 2);
    UpdateGUI();
}
...
void lms7002_mainPanel::OnEnableMIMOchecked(wxCommandEvent& event)
{
    // TODO:
    // uint16_t chBck;
    // LMS_ReadParam(sdrDevice, LMS7_MAC, &chBck);
    ...
    // }
    // LMS_WriteParam(sdrDevice, LMS7_MAC, chBck);
    // UpdateVisiblePanel();
}
