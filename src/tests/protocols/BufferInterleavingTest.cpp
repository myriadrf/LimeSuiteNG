...
template<typename... Ts> constexpr std::array<std::byte, sizeof...(Ts)> make_bytes(const Ts&&... args) noexcept
{
    return { std::byte(std::forward<const Ts>(args))... };
}
...
TEST(BufferInterleaving, SISO_I12_to_I12)
{
    ...
    constexpr std::array<std::byte, 6> expectedOutput{ make_bytes(0xFF, 0x37, 0x12, 0xBC, 0x0A, 0x80) };
...
TEST(BufferInterleaving, SISO_I16_to_I16)
{
    ...
    constexpr std::array<std::byte, 8> expectedOutput{ make_bytes(0xFF, 0x7F, 0x34, 0x12, 0xCD, 0xAB, 0x00, 0x80) };
    EXPECT_EQ(bytesProduced, expectedOutput.size());
    EXPECT_EQ(output, expectedOutput);
}
...
TEST(BufferInterleaving, MIMO_I16_to_I16)
{
    ...
    constexpr std::array<std::byte, 16> expectedOutput{ make_bytes(
...
TEST(BufferInterleaving, MIMO_I16_to_I12)
{
    ...
    constexpr std::array<std::byte, 12> expectedOutput{ make_bytes(
        0x10, 0x00, 0x02, 0x00, 0x01, 0x20, 0x30, 0x00, 0x04, 0x00, 0x03, 0x40) };
    EXPECT_EQ(bytesProduced, expectedOutput.size());
    EXPECT_EQ(output, expectedOutput);
}
...
TEST(BufferInterleaving, SISO_F32_to_I16)
{
    ...
    constexpr std::array<std::byte, 8> expectedOutput{ make_bytes(0xFF, 0x7F, 0x01, 0x80, 0xFF, 0x3F, 0x01, 0xE0) };
    EXPECT_EQ(bytesProduced, expectedOutput.size());
    EXPECT_EQ(output, expectedOutput);
}
...
{
    constexpr std::array<std::byte, 6> src{ make_bytes(0xFF, 0x37, 0x12, 0xBC, 0x0A, 0x80) };
...
{
    constexpr std::array<std::byte, 8> src{ make_bytes(0xFF, 0x7F, 0x34, 0x12, 0xCD, 0xAB, 0x00, 0x80) };
...
{
    constexpr std::array<std::byte, 8> src{ make_bytes(0xFF, 0x7F, 0x00, 0x80, 0xFF, 0x3F, 0x00, 0xE0) };
...
{
    constexpr std::array<std::byte, 16> src{ make_bytes(
        0x0A, 0x01, 0x0A, 0x02, 0x0B, 0x10, 0x0B, 0x20, 0x0A, 0x03, 0x0A, 0x04, 0x0B, 0x30, 0x0B, 0x40) };
