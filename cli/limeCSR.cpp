...
static std::string ReadFile(const std::string& fileName)
{
    ...
    {
        cerr << "ERROR: Not a readable input file: "sv << fileName << endl;
        exit(EXIT_FAILURE);
    }
        ...
        if (write)
        {
            for (uint64_t i = 0; i < wr_data.size(); i += 2)
            {
                ...
                if (status != OpStatus::Success)
                    cerr << "ERROR: CSR write failed for register address 0x" << hex << setw(16) << wr_data[i] << dec
                         << " with error: " << ToString(status) << endl;
            }
        }
        else if (read)
        {
            ...
            for (uint64_t i = 0; i < wr_data.size(); ++i)
            {
                ...
                if (status != OpStatus::Success)
                    cerr << "ERROR: CSR read failed for register address 0x" << hex << setw(16) << wr_data[i] << dec
                         << " with error: " << ToString(status) << endl;
            }
        }
