...
Pinning sub-packages
--------------------

Since metapackage depends on sub-packages, built sub-packages must be pinned in the metapackage requirements ``run:`` sub-section using ``- {{ pin_subpackage('sub_package_name', exact=True) }}`` or ``- {{ pin_subpackage('sub_package_name', max_pin='x.x') }}`` expression. Excerpt from Lime Suite NG metapackage ``meta.yaml`` file:

.. code-block:: yaml
   
   ...
   run:
  # This metapackage depends on the following Lime Suite NG sub-packages
