...
The Lime Suite NG metapackage sub-packages must be pinned to metapackage in the metapackage requirements ``run:`` sub-section using ``- {{ pin_subpackage('sub_package_name', exact=True) }}`` or ``- {{ pin_subpackage('sub_package_name', max_pin='x.x') }}`` expression. Excerpt from Lime Suite NG metapackage ``meta.yaml`` file:
