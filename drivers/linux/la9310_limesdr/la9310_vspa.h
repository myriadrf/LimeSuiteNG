...
enum vspa_power { VSPA_POWER_DOWN = 0, VSPA_POWER_UP, VSPA_POWER_CYCLE };
...
struct vspa_event_read {
    ...
    #define isaligned(ptr, align) (((ptr) & (align - 1)) == 0)
    ...
    #define ERR(...) \
        { \
            if (vspadev->debug & DEBUG_MESSAGES) \
    ...
    #define STATUS_REG_IRQ_VCPU_MSG (STATUS_REG_IRQ_VCPU_READ_MSG | STATUS_REG_IRQ_VCPU_SENT_MSG)
    ...
    #define STATUS_REG_IRQ_NON_GEN \
        (STATUS_REG_IRQ_FLAGS1 | STATUS_REG_IRQ_FLAGS0 | STATUS_REG_IRQ_DMA_COMP | STATUS_REG_IRQ_DMA_ERR | \
