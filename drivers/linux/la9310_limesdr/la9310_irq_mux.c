...
static int la9310_init_irq_mux(struct irq_evt_regs* irq_evt_regs, struct la9310_irq_mux_pram* irq_mux)
{
    u32 num_irq_words = la9310_mem_r(&irq_evt_regs->irq_evt_cfg) & NIRQ_WORDS_MASK;
...
static ssize_t la9310_show_irq_mux_stats(void* irq_mux_stats, char* buf, struct la9310_dev* la9310_dev)
{
    int print_len;
    struct la9310_irq_mux_stats* irq_stats = (struct la9310_irq_mux_stats*)irq_mux_stats;
    ...
    print_len += sprintf((buf + print_len), "num_msg_unit_irq_evt_raised: %7lu\n", irq_stats->num_msg_unit_irq_evt_raised);
...
static void la9310_set_imask(struct la9310_irq_mux_pram* irq_mux, int irq, u32 set_flg)
{
    ...
    irq_evt_en_reg = la9310_mem_r(irq_mux->irq_evt_en_reg + (irq_bit / LA9310_EVT_BTS_PER_WRD));
    if (set_flg)
        set_bit((irq_bit % LA9310_EVT_BTS_PER_WRD), (void*)&irq_evt_en_reg);
    else
        clear_bit((irq_bit % LA9310_EVT_BTS_PER_WRD), (void*)&irq_evt_en_reg);
    la9310_mem_w(irq_evt_en_reg, irq_mux->irq_evt_en_reg + (irq_bit / LA9310_EVT_BTS_PER_WRD));
}
...
static void la9310_set_irq_chip(struct la9310_irq_mux_pram* irq_mux)
{
    ...
    for (i = 0; i < irq_mux->num_irq; i++)
    {
        ...
        /* Setup irq  */
        irq_set_chip_and_handler(irq, &la9310_irq_chip, handle_simple_irq);
...
static void create_virq_bit_map(struct la9310_dev* la9310_dev, struct la9310_irq_mux_pram* irq_mux)
{
    ...
    for (i = 0; i < IRQ_EVT_LAST_BIT; i++)
    {
        if (num_irq)
        {
            ...
            dev_dbg(la9310_dev->dev, "%s: virq_map: %p, evt: %d, virq: %d\n", __func__, virq_map, virq_map->evt, virq_map->virq);
        }
        else
        {
...
static int la9310_init_irq_stats(struct la9310_dev* la9310_dev, struct la9310_irq_mux_pram* irq_mux)
{
...
static int la9310_probe_irq_mux(struct la9310_dev* la9310_dev, struct irq_evt_regs* irq_evt_regs)
{
    ...
    if (irq_mux == NULL)
        return -ENOMEM;
    irq_mux->virq_map = kzalloc((sizeof(struct virq_evt_map) * num_irq), GFP_KERNEL);
    ...
    {
        dev_err(la9310_dev->dev, "Failed to allocate IRQ irq_base:	%d\n", irq_mux->irq_base);
...
static void la9310_irq_processed(int irq, void* dev)
{
    ...
    mem = (la9310_dev->mem_regions[LA9310_MEM_REGION_CCSR].vaddr + MSG_UNIT_OFFSET);
    ccsr_mur = (struct la9310_msg_unit*)mem;
    ccsr_mur += LA9310_MSG_UNIT_1;

    irq_evt_clr_reg = la9310_mem_r(irq_mux->irq_evt_clr_reg + (irq_bit / LA9310_EVT_BTS_PER_WRD));
    set_bit((irq_bit % LA9310_EVT_BTS_PER_WRD), (void*)&irq_evt_clr_reg);
    la9310_mem_w(irq_evt_clr_reg, irq_mux->irq_evt_clr_reg + (irq_bit / LA9310_EVT_BTS_PER_WRD));
...
static irqreturn_t la9310_irq_handler(int irq, void* dev)
{
    ...
    int sts_bits;
    int match = 0;
    int num_words = la9310_mem_r(irq_mux->irq_evt_cfg_reg) & NIRQ_WORDS_MASK;
    u32 num_irq = irq_mux->num_irq;
    u32 irq_evt_sts_reg;
    u32 irq_evt_en_reg;
    int irq_base;
    ...
    {
        sts_bits = (num_irq < LA9310_EVT_BTS_PER_WRD) ? num_irq : LA9310_EVT_BTS_PER_WRD;
    ...
    if (!match)
        dev_warn(la9310_dev->dev, "No irq_mux line matched irq %d\n", irq);
...
int la9310_request_irq(struct la9310_dev* la9310_dev, struct irq_evt_regs* irq_evt_regs)
{
    ...
    if (!num_irq)
    {
        dev_err(la9310_dev->dev, "Invalid la9310 config! 'num irq 0'\n");
        return -EINVAL;
    }
    err = la9310_probe_irq_mux(la9310_dev, irq_evt_regs);
    if (!err)
    {
        err = request_irq(la9310_get_msi_irq(la9310_dev, MSI_IRQ_MUX), la9310_irq_handler, 0, la9310_dev->name, (void*)la9310_dev);
        if (!err)
        {
        ...
        {
            irq_free_descs(la9310_dev->la9310_irq_priv->irq_base, num_irq);
